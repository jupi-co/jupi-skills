...
description: "Surface prior decisions recorded in Jupi (the team's decision platform) that relate to what the user is working on, and report their insights, chosen outcomes, and any contradictions with the current direction. Use whenever the user is doing ideation or strategy work of any kind — brainstorming, exploring or shaping a new feature, concept, plan, or direction; product, but also GTM, pricing, positioning, hiring, org design, ops, partnerships, fundraising — or loading context for a piece of work: starting a project, drafting a spec/PRD/strategy memo, prepping for a meeting, reviewing an architectural, product, or business choice. Also use when the user asks anything like \"have we decided X?\", \"what did we choose for Y?\", \"is there prior art on Z?\", \"why did we go with…\". Lean toward using it proactively the moment ideation begins, before ideas harden — it keeps new ideas compatible with settled ones and catches conflicts early. Read-only — it never changes anything in Jupi."
---
