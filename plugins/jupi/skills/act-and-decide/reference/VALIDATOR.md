...
7. **Title** — the question / subject only. RETURN if it **embeds the options** (*"X or Y?"*, *"A vs B"*, listing the choices) or uses a **dash / hyphen as a separator**.
8. **No recommendation** — RETURN if any option is tagged *[reco]* / *recommended* / *best*, or if the decision steers toward one option. Options are neutral; the user decides.
...
**Decomposition — also check.** If the action is blocked by a **missing input** or a **method choice** that got glossed over (hand-waved, or wrongly treated as Case 0), **RETURN** — each blocker owes its own decision, and Jupi should **propose the likely answer as options**, not ask an open question. A "we don't know how" (e.g. how to sign) is never a dead end — it must become an *« ask X »* option. And check every option's action **spells out the concrete output** (the drafted email/doc for that option) and names its **prerequisites** (*« once A, B, C are settled, Jupi will… »*), never a vague "we'll do it then".

**Messaging voice & minimalism — also check.** For any action that **sends an email/message** (and for a Case-0 drafted message), verify the producer actually read the **recent history with that person in that same channel** (≥10 last messages we sent them) and that the draft **mirrors that register** — greeting, sign-off, tone, language (FR/EN), typical length — instead of a generic template. And verify it's **minimal**: no filler, no over-explaining, no longer than the job needs. RETURN if the voice isn't grounded in real past exchanges in that channel, or if the message is verbose. *(Carve-out: if the channel is **inaccessible** — WhatsApp, phone — the producer just flags it and matches the general register; don't RETURN for that.)*
